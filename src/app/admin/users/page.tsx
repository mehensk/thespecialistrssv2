  ...
  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
        <h1 className="text-2xl sm:text-3xl font-semibold text-[#111111]">User Management</h1>
        <Link
          href="/admin/users/new"
          className="bg-gradient-to-r from-[#1F2937] to-[#111111] text-white px-6 py-3 rounded-md hover:from-[#1A232E] hover:to-[#0F1419] transition-all duration-300 font-medium flex items-center gap-2 w-full sm:w-auto justify-center"
        >
          <Plus size={20} />
          Add User
        </Link>
      ...
      <div className="md:hidden space-y-3">
        {users.map((user) => (
          <div key={user.id} className="bg-white rounded-xl shadow-sm border border-[#E5E7EB] p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-base font-semibold text-[#111111] truncate">{user.name || 'No name'}</p>
                <p className="text-sm text-[#111111]/70 truncate">{user.email}</p>
              </div>
              <span
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-[#111111]/80">
              <div>Listings: {user._count.listings}</div>
              <div>Blogs: {user._count.blogPosts}</div>
              <div className="col-span-2 text-[#111111]/60">
