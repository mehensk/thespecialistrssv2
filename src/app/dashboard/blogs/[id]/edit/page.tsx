  ...
  useEffect(() => {
    async function fetchBlog() {
      try {
        ...
        // Remove img tags from content to get clean content for editing
        // The API will re-insert images when saving
        const cleanContent = blog.content
          .replace(/<img[^>]+>/gi, '') // Remove all img tags
          .replace(/\n\s*\n\s*\n+/g, '\n\n') // Clean up multiple newlines
          .trim();

        // Use images from the database (first one is cover)
        const allImages = blog.images && Array.isArray(blog.images) && blog.images.length > 0 
          ? [...blog.images] 
          : [];
        ...
        setFormData({
          ...
          images: allImages,
          coverPhotoIndex: 0, // First image is always cover
        });
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('Blog update error:', data);
        setLoading(false);
        return;
      }
    ...
    return (
      <div>
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-semibold text-[#111111]">Edit Blog Post</h1>
        </div>
        <div className="bg-white rounded-xl shadow-lg border border-[#E5E7EB] p-8">
          <div className="text-center py-8">
            <Loader2 className="animate-spin mx-auto mb-4 text-[#1F2937]" size={32} />
    ...
    <div>
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-md p-4">
        <p className="text-sm text-blue-800">
          <strong>Note:</strong> After updating, your blog post will need admin approval again if it was previously unpublished.
        </p>
      </div>
    </div>
  );
